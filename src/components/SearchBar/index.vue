  ...
  <div class="searchbar-container">
    <div class="search-input">
      <el-input placeholder="搜索" v-model="filter.value"
        ...
        @keydown.down.native="selectDown" @keyup.up.native="selectUp" style="width:100%;">
      </el-input>
    ...
    <div class="search-btn">
      <el-button type="primary" icon="el-icon-edit" @click.native="add"></el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    selectDown () {
      ...
      console.log('SearchBar.vue搜索项下移:' + JSON.stringify(filterItem))
    ...
    selectUp () {
      ...
      let filterItem = this.items.filter(t => t.id === this.now)
      console.log('SearchBar.vue搜索项上移:' + JSON.stringify(filterItem))
      if (filterItem.length > 0) {
        this.filter = filterItem[0]
      }
    },
    ...
    selectClick (index) {
      ...
      console.log('SearchBar.vue搜索项选择事件:' + JSON.stringify(filterItem))
      if (filterItem.length > 0) {
        this.filter = filterItem[0]
      }
      this.items = []
